...
def test_the_recorded_case_is_what_was_recorded():
    ...
    assert (order.hours_since_purchase, order.status) == (11, "unused")
    assert policy.window_hours == 24
    assert policy.source_url == "https://openrouter.zendesk.com/hc/en-us/articles/40858600529307-Refunds-and-Payment-Information"
    assert "within 24 hours of purchase" in policy.text


def test_the_escalating_case_is_outside_the_window_and_over_the_threshold():
    _, order, policy = load(TWO)
    assert order.hours_since_purchase > policy.window_hours
